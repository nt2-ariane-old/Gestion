...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      let individu = await Individu.findOne({ courriel: email, estEmploye: true })
...
router.get("/me", auth, async (req, res) => {
  try {
    ...
    const fiche = await Individu.findById(user.id_fiche)
    res.json({ user: user, fiche: fiche });
  } catch (e) {
  ...
  User.findOne({
    ...
    if (user === null) {
      ...
      let mailOptions = {
        ...
        text:
          'Vous recevez ce courriel car vous (ou quelqu\'un d\'autres) as demandé de changer votre mot de passe pour le site gestion.nt2.uqam.ca\n\n'
          + 'Si vous avez faites la demandes, veuillez vous connecter à l\'adresse suivante : \n\n'
          + `http://www.doutreguay.com/reset/${token} \n\n`
...
router.put('/addSlackInfo', (req, res, next) => {
  User.findOne({ _id: req.body.user_id }).then(async user => {
    if (user !== null) {
      
      user.slack = req.body.slack
      console.log(user)
      await user.save().then(() => {
        return res.status(200).send('Le contenu Slack à été ajouté')
      });
    }
    else {
      return res.status(404).send('L\'utilsateur n\'existe pas')
...
router.get('/slackInfo', (req, res, next) => {
  const id = req.header("user_id")
  console.log(id)
  User.findOne({ _id: id }).then(async user => {
    if (user !== null) {
      return res.status(200).send(user.slack)
    }
    else {
      return res.status(404).send('L\'utilsateur n\'existe pas')
    }
